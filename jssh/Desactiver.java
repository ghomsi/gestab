 ...
 * @author ghomsi
 */
public class Desactiver {
    public Desactiver(ActionEvent t,DAOequipement dao,Port port,Client client,String user,String passwd){
        JSch jsch = new JSch();
        //String[] frac = dao.LireFichier("infos_comptes.ssh",label);
        //String user = frac[2];            //CHANGE ME
        //tring host = frac[0]; //CHANGE ME
        ...
        ps.println("desactivate "+port.getNumeroPort());
        ps.println("save");
        ps.println("quit");
        ps.println("quit");
        ps.close();

        printResult(input, channel,client);
        ...
        session.disconnect();
            ...
            Logger.getLogger(Desactiver.class.getName()).log(Level.SEVERE, null, ex);
        } catch (Exception ex) {
            Logger.getLogger(Desactiver.class.getName()).log(Level.SEVERE, null, ex);
        }
   ...
   private static void printResult(InputStream input,
                                   Channel channel,Client client) throws Exception
   {
      int SIZE = 1024;
      ...
      while (true)
      {
         ...
         if(channel.isClosed())
         {
            ...
            new Notifier("GESTAB",client.getNom()+" "+client.getPrenom()+" Suspendu",2);
            break;
         }
         try
         {
